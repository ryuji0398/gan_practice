...
class Generator(nn.Module):
    def __init__(self, ngf=64, nz=100, nc=3, im_size=1024, config_txt=False):
        ...
        # SPADE_generator
        self.feat_8   = SPADEResnetBlock(1024, 512, self.opt)
        self.feat_16  = SPADEResnetBlock(512, 256, self.opt)
        self.feat_32  = SPADEResnetBlock(256, 128, self.opt)
        self.feat_64  = SPADEResnetBlock(128, 128, self.opt)
        self.feat_128 = SPADEResnetBlock(128, 64, self.opt)
        self.feat_256 = SPADEResnetBlock(64, 32, self.opt)
        ...
        if im_size > 256:
            ...
            # SPADE_generator
            # self.feat_512 = SPADEResnetBlock(32, 16, self.opt)
    ...
    def forward(self, input, seg):
        ...
        feat_8   = self.up(feat_8)

        feat_16  = self.feat_16(feat_8, seg)
        feat_16  = self.up(feat_16)

        feat_32  = self.feat_32(feat_16, seg)
        feat_32  = self.up(feat_32)

        feat_64  = self.feat_64(feat_32, seg)
        feat_64  = self.se_64( feat_4, self.up(feat_64) )

        feat_128 = self.feat_128(feat_64, seg)
        feat_128 = self.se_128( feat_8, self.up(feat_128) )

        feat_256 = self.feat_256(feat_128, seg)
        feat_256 = self.se_256( feat_16, self.up(feat_256) )
        ...
        feat_512 = self.se_512( feat_32, self.feat_512(feat_256) )
        # feat_512 = self.feat_512(feat_256, seg)
        # feat_512 = self.se_512( feat_32,  self.up(feat_512) )
